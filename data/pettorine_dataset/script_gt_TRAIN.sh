...
cd pettorine
find -type f -name '*' | sed -n '1,14000p' | cut -c18-21 > ../temp/labels_true.txt
find `pwd` -type f -name '*' | sed -n '1,14000p' > ../temp/filenames_true.txt
paste -d ' ' ../temp/filenames_true.txt ../temp/labels_true.txt > ../temp/file_listTRAIN.txt 

cd ../no_pettorine
find `pwd` -type f -name '*' | sed -n '1,15p' > ../temp/filenames_false.txt
